(() => {
  ...
  socket.on("lobby-update", (data) => {
    ...
    document.getElementById("setting-similar-mode").checked = !!settings.similarMode;
    document.getElementById("setting-similar-mode").disabled = !isHost;
    currentSimilarity = settings.similarity || "medium";
    document.querySelectorAll("#similarity-segmented .segmented-btn").forEach((btn) => {
      btn.classList.toggle("selected", btn.dataset.value === settings.similarity);
      btn.disabled = !isHost;
    });
    document.getElementById("similarity-row").classList.toggle("hidden-disabled", !settings.similarMode);

    document.getElementById("setting-blind-imposter").checked = !!settings.blindImposter;
    document.getElementById("setting-blind-imposter").disabled = !isHost;
    document.getElementById("blind-imposter-row").classList.toggle("hidden-disabled", !settings.similarMode);
  ...
  function pushSettings() {
    ...
    settingsDebounce = setTimeout(() => {
      socket.emit("update-settings", {
        ...
        similarMode: document.getElementById("setting-similar-mode").checked,
        similarity: currentSimilarity,
        blindImposter: document.getElementById("setting-blind-imposter").checked,
      });
    }, 150);
  }
  ...
  document.getElementById("setting-similar-mode").addEventListener("change", () => {
    const on = document.getElementById("setting-similar-mode").checked;
    document.getElementById("similarity-row").classList.toggle("hidden-disabled", !on);
    document.getElementById("blind-imposter-row").classList.toggle("hidden-disabled", !on);
    pushSettings();
  });
  document.getElementById("setting-blind-imposter").addEventListener("change", pushSettings);
  document.querySelectorAll("#similarity-segmented .segmented-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      if (!isHost) return;
      currentSimilarity = btn.dataset.value;
      document.querySelectorAll("#similarity-segmented .segmented-btn").forEach((b) => b.classList.remove("selected"));
      btn.classList.add("selected");
      pushSettings();
    });
  });
  ...
  function buildCanvasGrid() {
    ...
    players.forEach((p) => {
      ...
      avatar.dataset.voteLabel = `VOTE ${p.name.toUpperCase()} AS IMPOSTER`;
      const nameEl = document.createElement("span");
      nameEl.className = "p-name";
      nameEl.textContent = p.name + (p.id === myId ? " (you)" : "");
  ...
  socket.on("game-start", (data) => {
    ...
    if (isImposter) {
      ...
      if (data.similarMode && data.word) {
        phaseLabelEl.textContent = "YOU'RE THE IMPOSTER \u2014 DRAW THIS";
        wordEl.textContent = data.word;
      } else {
        phaseLabelEl.textContent = "DRAW THE WORD";
  ...
  socket.on("reveal-result", (data) => {
    ...
    const impWordEl = document.getElementById("reveal-imposter-word");
    if (data.imposterWord) {
      document.getElementById("reveal-imposter-word-val").textContent = data.imposterWord;
      impWordEl.classList.remove("hidden");
    } else {
      impWordEl.classList.add("hidden");
    }
